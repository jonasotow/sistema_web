...
class Estadoscta_Comprobantes extends MY_Controller {

    public function __construct(){
        parent::__construct();
        $this->load->model('estadoscta/comprabantes_model','oComprobantes');
        ...
        $this->template['titulo'] = 'Comprobantes Fiscales Vimifos';
        $this->template['usuario'] = $this->cliente->usu_nombre;
        $this->template['url'] = site_url('comprobantes/index');
    ...
    public function index()
    {
        ...
        $config['base_url'] = site_url('comprobantes/index');
        $config['first_url'] = site_url('comprobantes/index/0/'.$this->uri->segment(4));
            ...
            if($rows_facturas){
                foreach($rows_facturas as $row) { 
                    $facturas .= "<tr>
                                            ...
                                            <td><a href='".site_url('comprobantes/generar_pdfG/')."/".$row->id_Comprobante."' target='_blank' class='fa fa-file-pdf-o fa-lg'>&nbsp;</a></td>
                                            <td><a href='".site_url('comprobantes/generar_zipG/')."/".$row->id_Comprobante."' target='_blank' class='fa fa-file-archive-o fa-lg'>&nbsp;</a></td>
                                        </tr>";
                }
            }else{
            ...
            if($rows_facturas){
                foreach($rows_facturas as $row) { 
                    $facturas .= "<tr>
                                            ...
                                            <td>$".number_format($row->subTotal,2)."</td>
                                            <td>$".number_format($row->total,2)."</td>
                                            <td><a href='".site_url('comprobantes/generar_pdf/')."/".$row->id_Comprobante."' target='_blank' class='fa fa-file-pdf-o fa-lg'>&nbsp;</a></td>
                                            <td><a href='".site_url('comprobantes/generar_zip/')."/".$row->id_Comprobante."' target='_blank' class='fa fa-file-archive-o fa-lg'>&nbsp;</a></td>
                                        </tr>";
                }
            }else{
                $facturas = "<tr><td>#</td><td align='center' colspan='7'>No se encontraron registros</td></tr>";
    ...
    public function generar_pdf($idcomprobante){
        ...
        $pdf = "";
        $pdf .= ($row->serie == "CN" || $row->serie == "CLO") ? 0 : $row->estatus;
        $pdf .= "-".$row->serie.$row->folio;
        ...
        if($row->folio <= 9)
            $ceros_nuevo="00000000000";
        if($row->folio > 9)
            $ceros_nuevo="0000000000";
        if($row->folio >= 100)
            $ceros_nuevo="000000000";
        if($row->folio >= 1000)
            $ceros_nuevo="00000000";

        $pdf_nuevo .= $row->estatus."VIM970403775-".$row->serie.$ceros_nuevo.$row->folio;          
        $pdf_nuevo = trim($pdf_nuevo);   

        if($factura_fld){
            if(file_exists("/home/vimpdf/".$pdf_nuevo.".pdf")){
                $rutav = "/home/vimpdf/".$pdf_nuevo.".pdf";
                $nom_pdf = $pdf_nuevo;
            }else{
                $rutav = "/home/parametros/".$pdf.".pdf";
                $nom_pdf = $pdf;
            }
            ...
            $nombre_pdf = $row->estatus."VIM970403775-bd-".$row->serie.$row->folio;
