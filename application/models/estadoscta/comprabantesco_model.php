...
class Comprabantesco_Model extends My_Model {
    public $table;
    ...
    public function facturas_antiguas($clienterfc,$idcomprobante){
        $db = $this->load->database('businesscfdi', TRUE); 
        $db->select("co.estatus,co.serie,co.folio"); 
    ...
    public function Contenido($por_pagina,$segmento,$clienterfc,$param) {
        ...
        $sql = "SELECT * FROM (SELECT a.*,row_number() OVER (ORDER BY a.fecha DESC) AS row
            ...
            WHERE re.rfc = ? AND (em.rfc = ?) AND year(co.fecha) >= ? AND co.folio like '%$param%') AS a) AS b WHERE b.row >= $var1 and b.row <= $var2";
        $query = $db->query($sql, array($clienterfc,'2012','6',$clienterfc,'EFE060511IA7','2012'));
        if($query->num_rows()>0)
            $array = $query->result();
        else
            $array = "";
   ...
   public function filas($clienterfc,$param){
        ...
        $db2->where("re.rfc = '$clienterfc' and (em.rfc = 'EFE060511IA7') and year(co.fecha) >= '2012'");
